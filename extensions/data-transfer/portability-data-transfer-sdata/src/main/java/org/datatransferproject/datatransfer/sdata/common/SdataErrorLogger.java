...
/**
 * Helper class that provides functionality for creating errors & logging them to data store. This
 * class logs errors that should appear as failures in the completion email.
 */
public class SdataErrorLogger {
   ...
   */
  // TODO: this is from  org.datatransferproject.datatransfer.google.common.GoogleErrorLogger;
  // refactor it to be a common package somwehere so its not defined locally.
  public static ErrorDetail createErrorDetail(
      String idempotentId, String title, Exception e, boolean canSkip) {
    return ErrorDetail.builder()
        .setId(idempotentId)
        .setTitle(title)
        .setException(getStackTraceAsString(e))
        .setCanSkip(canSkip)
        .build();
  }
}
